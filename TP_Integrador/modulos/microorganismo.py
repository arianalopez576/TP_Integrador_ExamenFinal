...
#composicion con cromosoma
#agregacion con parametros de simulacion
#dependencia con gestor alimento
class Microorganismo:
    ...
    def __init__(self, p_parametros_simulacion):
        ...
        self.__fila = random.randint(0, p_parametros_simulacion.dic_parametros['max_filas']-1) #valor para que no se vaya del rango del territorio
        self.__columna = random.randint(0, p_parametros_simulacion.dic_parametros['max_columnas']-1)
        self.__energia = p_parametros_simulacion.dic_parametros['energia_inicial']
        self.__mov_relativo = p_parametros_simulacion.mov_relativo #el mov relativo no esta dentro del dic, ¿se llama de la misma forma?
        self.__energia_perdida = p_parametros_simulacion.dic_parametros['energia_perdida']
        self.__prob_mutacion = p_parametros_simulacion.dic_parametros['prob_mutacion']
        self.__prob_cambio_MO = p_parametros_simulacion.dic_parametros['prob_cambio_MO']
        self.__max_alimento_ingerir = p_parametros_simulacion.dic_parametros['cant_max_alimento_ingerir']
        self.__energia_max = p_parametros_simulacion.dic_parametros['energia_max']
    ...
    def reproduccion(self, MO_padre):
        ...
        MO_hijo = Microorganismo(self.__parametros)
        ...
        if self.get_energia() != 0:
            ...
            if celda_alimento != -1:
                ...
                #si la posicion esta fuera de rango, busca otra
                    ...
                    if p_epoca % self.__parametros.dic_parametros['prob_cambio_MO'] == 0: #random.random da un num decimal aleatorio entre 0 y 100
    ...
    def __verificar_ubicacion(self, p_fila_nueva, p_columna_nueva):
        if (p_fila_nueva >= 0 and p_fila_nueva < self.__parametros.dic_parametros['max_filas']) and (p_columna_nueva >= 0 and p_columna_nueva < self.__parametros.dic_parametros['max_columnas']):
            return True 
        else: 
            False
